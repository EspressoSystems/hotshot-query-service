...
///
/// let qs1 = QueryServiceProvider::new("https://backup.query-service.1".parse()?, Base);
/// let qs2 = QueryServiceProvider::new("https://backup.query-service.2".parse()?, Base);
...
mod test {
    ...
    async fn test_fetch_first_provider_fails() {
        ...
        app.register_module(
            "availability",
            define_api(&Default::default(), vbs::version::StaticVersion::<0, 1> {}).unwrap(),
        )
        .unwrap();
