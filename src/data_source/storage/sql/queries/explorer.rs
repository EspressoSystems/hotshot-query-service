    ...
    async fn get_transaction_summaries(
        ...
        let transaction_target_query = match target {
            TransactionIdentifier::Latest => query(
                "SELECT t.block_height AS height, t.idx AS \"index\" FROM \"transaction\" AS t ORDER BY (t.block_height, t.idx) DESC LIMIT 1",
            ),
            TransactionIdentifier::HeightAndOffset(height, _) => query(
                "SELECT t.block_height AS height, t.idx AS \"index\" FROM \"transaction\" AS t WHERE t.block_height = $1 ORDER BY (t.block_height, t.idx) DESC LIMIT 1",
            )
            .bind(*height as i64),
            TransactionIdentifier::Hash(hash) => query(
                "SELECT t.block_height AS height, t.idx AS \"index\" FROM \"transaction\" AS t WHERE t.hash = $1 ORDER BY (t.block_height, t.idx) DESC LIMIT 1",
            )
            .bind(hash.to_string()),
        };
                ...
                "SELECT {BLOCK_COLUMNS}
                        ...
                        WHERE h.height IN (
                            SELECT t.block_height
                                ...
                                ORDER BY (t.block_height, t.idx) DESC
                                LIMIT {}
                        )
                        ORDER BY h.height DESC",
                ...
                "SELECT {BLOCK_COLUMNS}
                    ...
                    WHERE h.height = (
                        SELECT t1.block_height
                            FROM \"transaction\" AS t1
                            WHERE t1.hash = {}
                            ORDER BY (t1.block_height, t1.idx) DESC
                            LIMIT 1
                    )
                    ORDER BY h.height DESC",
                query.bind(hash.to_string())?,
