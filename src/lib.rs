...
//! use hotshot_types::constants::{Base, Base};
...
//!
//! // Create API modules.
//! let availability_api = availability::define_api(&Default::default(), Base)
//!     .map_err(Error::internal)?;
//! let node_api = node::define_api(&Default::default(), Base)
//!     .map_err(Error::internal)?;
//! let status_api = status::define_api(&Default::default(), Base)
//!     .map_err(Error::internal)?;
//!
...
//!
//! // Serve app.
//! spawn(app.serve("0.0.0.0:8080", Base));
//!
...
//! spawn(run_standalone_service(options, data_source, hotshot, Base));
//! # Ok(())
//! # }
//! ```
//!
...
mod test {
    ...
    async fn test_composition() {
        ...
        app.register_module(
            "availability",
            availability::define_api(&Default::default(), vbs::version::StaticVersion::<0, 1> {})
                .unwrap(),
        )
        .unwrap()
        .register_module(
            "node",
            node::define_api(&Default::default(), vbs::version::StaticVersion::<0, 1> {}).unwrap(),
        )
        .unwrap()
        .register_module(
            "status",
            status::define_api(&Default::default(), vbs::version::StaticVersion::<0, 1> {})
                .unwrap(),
        )
        .unwrap()
